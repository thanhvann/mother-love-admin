...
const Users: React.FC = () => {
    ...
    const filteredUsers = selectedRole === "ALL" ? users : users.filter(user => user.roleName === selectedRole);
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">First Login</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredUsers.map((user) => (
                            <tr key={user.userId}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.point}</td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <img className="h-10 w-10 rounded-full" src={user.image} alt={user.fullName} />
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.roleName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.firstLogin ? "Yes" : "No"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
